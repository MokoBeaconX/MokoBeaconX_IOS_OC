...
NSString *const MKEddystoneAdditionalInformation = @"MKEddystoneAdditionalInformation";
NSString *const MKEddystoneDataInformation = @"MKEddystoneDataInformation";
NSString *const MKEddystoneDataStatusLev = @"MKEddystoneDataStatusLev";

@interface MKEddystoneOperation()
...
/**
 线程ID
 */
@property (nonatomic, assign)MKEddystoneOperationID operationID;
...
@implementation MKEddystoneOperation
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    [self parseDataSuccess:[MKEddystoneDataParser parseReadDataFromCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    [self parseDataSuccess:[MKEddystoneDataParser parseWriteDataFromCharacteristic:characteristic]];
}
    ...
    if (self.completeBlock) {
        if (self.needPartOfData) {
            ...
            NSDictionary *resultDic = @{
                                        MKEddystoneAdditionalInformation:(self.additionalInformation ?: @{}),
                                        MKEddystoneDataInformation:self.dataList,
                                        //对于有附加信息的，lev为2，对于普通不包含附加信息的，lev为1.
                                        MKEddystoneDataStatusLev:(self.additionalInformation ? @"2" : @"1"),
                                        };
            if (self.completeBlock) {
...
- (void)parseDataSuccess:(NSDictionary *)dataDic{
    ...
    MKEddystoneOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == MKEddystoneOperationDefaultID || operationID != self.operationID) {
        return;
    }
    ...
    NSString *numString = returnData[MKEddystoneDataNum];
