...
@interface HCKScanSearchView()<UIGestureRecognizerDelegate>
...
@implementation HCKScanSearchView

- (void)dealloc{
    NSLog(@"HCKScanSearchView销毁");
}
...
- (HCKTextField *)textField{
    if (!_textField) {
        _textField = [[HCKTextField alloc] initWithValidationRules:normalInput];
        _textField.frame = CGRectMake(offset_X,
                                      offset_X,
                                      kScreenWidth - 4 * offset_X,
                                      30.f);
        _textField.borderStyle = UITextBorderStyleNone;
        _textField.font = HCKFont(13.f);
        _textField.textColor = DEFAULT_TEXT_COLOR;
        _textField.attributedPlaceholder = [HCKAttributedStringManager getAttributedString:@[@"Device name or mac address"] fonts:@[HCKFont(15.f)] colors:@[RGBCOLOR(222, 222, 222)]];
...
- (UIButton *)doneButton{
    if (!_doneButton) {
        ...
        [_doneButton.titleLabel setFont:HCKFont(16.f)];
...
- (UILabel *)rssiValueLabel{
    if (!_rssiValueLabel) {
        ...
        _rssiValueLabel.font = HCKFont(12.f);
