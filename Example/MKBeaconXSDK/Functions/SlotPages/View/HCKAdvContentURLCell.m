...
static NSString *const HCKAdvContentURLCellIdenty = @"HCKAdvContentURLCellIdenty";
static CGFloat const labelHeight = 30.f;

@interface HCKAdvContentURLCell()

@property (nonatomic, strong)UILabel *urlLabel;
@property (nonatomic, strong)UILabel *typeLabel;
@property (nonatomic, strong)HCKTextField *textField;
@property (nonatomic, strong)NSMutableArray *dataList;

@end

@implementation HCKAdvContentURLCell

+ (HCKAdvContentURLCell *)initCellWithTableView:(UITableView *)tableView{
    HCKAdvContentURLCell *cell = [tableView dequeueReusableCellWithIdentifier:HCKAdvContentURLCellIdenty];
    if (!cell) {
        cell = [[HCKAdvContentURLCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:HCKAdvContentURLCellIdenty];
    }
    return cell;
}
...
- (HCKTextField *)textField{
    if (!_textField) {
        _textField = [self createNewTextFieldWithRules:normalInput];
        _textField.attributedPlaceholder = [HCKAttributedStringManager getAttributedString:@[@"mokosmart.com/"] fonts:@[HCKFont(15.f)] colors:@[RGBCOLOR(222, 222, 222)]];
    }
    return _textField;
}
