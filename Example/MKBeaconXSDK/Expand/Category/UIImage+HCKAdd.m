...
#import "UIImage+HCKAdd.h"
#import "UIImage+YYAdd.h"

@implementation UIImage (HCKAdd)
...
+ (UIImage *)thumbnailWithImageWithoutScale:(UIImage *)image size:(CGSize)asize
{
    ...
    if (nil == image) {
        ...
        CGContextRef context = UIGraphicsGetCurrentContext();
        CGContextSetFillColorWithColor(context, [COLOR_CLEAR_MACROS CGColor]);
...
- (BOOL)saveToLocal:(NSString *)filePath
    ...
    NSString *docFilePath = [kLibraryPath stringByAppendingString:filePath];
    //创建文件夹
    NSFileManager *fileManager = [NSFileManager defaultManager];
    [fileManager createDirectoryAtPath:docFilePath
           withIntermediateDirectories:YES
                            attributes:nil
                                 error:nil];
    //返回保存图片的路径（图片保存在ImageFile文件夹下）
    NSString * imagePath = [docFilePath stringByAppendingPathComponent:imageName];
