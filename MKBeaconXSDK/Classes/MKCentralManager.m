...
@interface NSObject (beaconXSDK)

@end

@implementation NSObject (beaconXSDK)
...
static MKCentralManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKCentralManager()<CBCentralManagerDelegate,CBPeripheralDelegate>
...
@property (nonatomic, assign)MKEddystoneConnectStatus connectState;

@property (nonatomic, assign)MKEddystoneCentralManagerState managerState;

@property (nonatomic, assign)MKEddystoneLockState lockState;
...
+ (MKCentralManager *)sharedInstance{
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [[MKCentralManager alloc] init];
        }
    });
    return manager;
}
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI{
    ...
    dispatch_async(_centralManagerQueue, ^{
        MKBaseReceiveBeacon *beaconModel = [MKReceiveiBeacon fecthBeaconWithAdvData:advertisementData];
        if (!beaconModel) {
            return ;
        }
        ...
        if ([self.scanDelegate respondsToSelector:@selector(didReceiveBeacon:manager:)]) {
            moko_main_safe(^{
                [self.scanDelegate didReceiveBeacon:beaconModel manager:manager];
            });
        }
    });
}
...
- (void)startScanPeripheral{
    ...
    [self.centralManager scanForPeripheralsWithServices:@[[CBUUID UUIDWithString:MKEddystoneServiceID],
                                                          [CBUUID UUIDWithString:MKPeripheralServiceID],
                                                          [CBUUID UUIDWithString:MKiBeaconServiceID]]
                                                options:nil];
}
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    if (self.managerState != MKEddystoneCentralManagerStateEnable) {
...
- (void)connectSucBlock:(MKConnectSuccessBlock)sucBlock
    ...
    [self updateConnectState:MKEddystoneConnectStatusConnecting];
...
- (void)connectPeripheralFailed:(BOOL)isPasswordError{
    ...
    self.peripheral = nil;
    [self updateConnectState:MKEddystoneConnectStatusConnectedFailed];
    if (isPasswordError) {
...
- (void)connectPeripheralSuccess{
    ...
    [self resetOriSettings];
    [self updateConnectState:MKEddystoneConnectStatusConnected];
    if (self.sucBlock) {
...
- (void)updateConnectState:(MKEddystoneConnectStatus)state{
    self.connectState = state;
    if ([self.stateDelegate respondsToSelector:@selector(peripheralConnectStateChanged:manager:)]) {
        moko_main_safe(^{
            [self.stateDelegate peripheralConnectStateChanged:state manager:manager];
        });
    }
}
...
- (void)updateManagerState{
    ...
    if ([self.stateDelegate respondsToSelector:@selector(centralStateChanged:manager:)]) {
        moko_main_safe(^{
            [self.stateDelegate centralStateChanged:managerState manager:manager];
        });
    }
}

- (void)updateLockState:(MKEddystoneLockState)lockState{
    self.lockState = lockState;
    if ([self.stateDelegate respondsToSelector:@selector(eddystoneLockStateChanged:manager:)]) {
        moko_main_safe(^{
            [self.stateDelegate eddystoneLockStateChanged:lockState manager:manager];
        });
    }
}
