...
static NSString *const HCKEddStoneTLMCellIdenty = @"HCKEddStoneTLMCellIdenty";
...
@implementation HCKEddStoneTLMCell

+ (HCKEddStoneTLMCell *)initCellWithTableView:(UITableView *)tableView{
    HCKEddStoneTLMCell *cell = [tableView dequeueReusableCellWithIdentifier:HCKEddStoneTLMCellIdenty];
    if (!cell) {
        cell = [[HCKEddStoneTLMCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:HCKEddStoneTLMCellIdenty];
    }
    return cell;
}
...
- (void)setBeacon:(MKReceiveTLMBeacon *)beacon{
    _beacon = nil;
    _beacon = beacon;
    if (!_beacon) {
        return;
    ...
    if (ValidNum(_beacon.temperature)) {
        NSString *string = [NSString stringWithFormat:@"%ld",(long)[_beacon.temperature integerValue]];
        [self.temperatureLabel setText:[NSString stringWithFormat:@"%@°C",string]];
    }
